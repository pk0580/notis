...
> Технический обзор сервиса: бизнес-задача, архитектура, поток данных, инфраструктура, ключевые алгоритмы. Для каждого алгоритма указаны конкретные классы и методы, в которых он реализован.
...
Покрывает функциональные и нефункциональные требования `TASK.md` — массовую рассылку, приоритезацию, статусы и историю доставки, персистентность очереди, at-least-once, идемпотентность входящих запросов, интеграционные тесты на полную цепочку, упаковку в `docker compose up`. Архитектурный тир — Complex (DDD): state-машина уведомления, инварианты на агрегате, асинхронная доставка через брокер.
...
│   │   │   ├── DispatchNotificationsAction.php     ← основной write-юзкейс (массовая рассылка)
...
│   │   ├── GetNotificationsByRecipientHandler.php  ← read-сторона (история по контакту)
...
int $attempts;                         // считает ТОЛЬКО НЕУДАЧНЫЕ попытки
...
|---|---|---|
| `markAsSent(ProviderMessageId)` | `Queued` | `status = Sent`, сохраняет `providerMessageId`, пишет переход в `history`. **`attempts` НЕ увеличивает** — поле отражает только число неудач (`attempts = 0` == «доставлено с первой попытки»). |
...
- **`MessageBody::for(Channel, string)`** — единственная фабрика. Длина читается из `config('notifications.body_max.{channel}')`: SMS — 1..1000 символов, Email — 1..10000. `TASK.md` лимит не задаёт; SMS-граница выбрана так, чтобы клиент не отправил по ошибке многосегментный SMS без понимания segment-биллинга. Domain-слой остаётся framework-free: если `function_exists('config')` ложен (юнит-тесты без Laravel boot), используется fallback-константа.
...
Сценарий покрывает четыре требования `TASK.md` одновременно: массовую рассылку, персистентность очереди в БД и брокере, at-least-once доставку, идемпотентность входящих запросов.
...
Семантика **all-or-nothing**: любой невалидный элемент → весь запрос отклонён с 422. Это намеренно: частичный приём усложнил бы контракт без бизнес-выгоды.
...
Алгоритм публикации реализован в две фазы, чтобы гарантировать надёжность и производительность:
...
3. Увеличивается счётчик `attempts`. Максимальное количество попыток публикации в брокер — 10 (после чего запись остаётся в outbox с `published_at IS NULL` и требует ручного вмешательства).
...
**Минимальный payload:** в AMQP идёт только `notification_id`. Тело сообщения, recipient, channel и т.д. живут в БД и подгружаются потребителем. Это исключает рассинхронизацию AMQP-снимка с актуальным состоянием.
...
Все exchanges и queues — `durable=true`. Все сообщения — `persistent=true`. После рестарта брокера данные не теряются — это и есть требование персистентности из `TASK.md`.
...
4. try:
   ...
   catch PermanentDeliverNotificationFailedException:  // GatewayRejected → dropped без retry
     Log::warning(...)
...
if ($n->status() !== Queued) return DeliverNotificationResult::NoOp;   ← exactly-once на бизнес-уровне
...
**Правило одного `save` на путь:** каждая ветка catch мутирует агрегат в памяти полностью, затем делает **один** `repo->save($n)`. Это исключает гонку с optimistic-lock между двумя последовательными `save` в одном handle.
...
Реализация — `ConsumeNotificationJob::handleRetry()`. Закрывает требование `TASK.md` об автоматическом повторе попыток отправки при временной недоступности шлюзов.
...
- `ConsumeNotificationJob` ловит permanent-исключение, делает `Log::warning(...)` и `$message->ack()`. **В DLQ не публикует** — статус уже `dropped`, повторная доставка бессмысленна.

---

## 10. Имитация колбэка delivered/dropped (без webhook)

`TASK.md` явно требует только заглушки шлюзов. Чтобы получить полную state-машину `Sent → Delivered/Dropped`, реализован внутренний асинхронный job — `SimulateDeliveryAckJob`. Без HTTP-цикла: имитация колбэка от провайдера выполняется через Laravel queue (database driver).
...
Это эмулирует поведение реальных шлюзов, дедуплицирующих по client-side message-id. Третья линия защиты exactly-once: даже если consumer как-то умудрится дважды вызвать `send()` с тем же `NotificationId`, шлюз вернёт тот же `ProviderMessageId`, а не отправит сообщение повторно.
...
## 12. Read API: история уведомлений подписчика
...
**`recipient_masked`** — сырой контакт в ответе не возвращается (клиент уже знает контакт — он сам его передал в query). Это снижает риск утечки PII в логах/трейсах потребителя.

---

## 13. Приоритезация — Bulkhead

`TASK.md`: «транзакционные обгоняют маркетинговые». Реализовано как **физическая изоляция** — раздельные очереди в брокере + раздельные пулы воркеров:
...
## 14. Идемпотентность входа

Уже описана в разделе "Рассылка уведомлений". Сводим контракт:
...
Отдельной таблицы `idempotency_keys` в Postgres нет — `TASK.md` прямо говорит «Redis для дедубликации».

---

## 15. Гарантия доставки
...
- **Transactional Outbox**: запись в `notifications` + `outbox_messages` в одной транзакции (см. раздел "Рассылка уведомлений"). Если приложение упадёт между `commit` БД и `basic_publish`, `OutboxPublisher` дочинит позже.

### 15.2 At-least-once

- Manual ack: `ConsumeNotificationJob` делает `$message->ack()` **только после** успешного `$repo->save($n)` со статусом `Sent`.
- Если воркер упал между `gateway->send()` и `save()` — AMQP-сервер переотдаст сообщение другому воркеру (или этому же после рестарта), `gateway->send()` будет вызван повторно — но это безопасно благодаря разделу "Дедупликация на шлюзе".
- `SimulateDeliveryAckJob` на database queue с `--tries=3` тоже at-least-once.

### 15.3 Exactly-once на бизнес-уровне (бонус)
...
Дополнительно — дедупликация шлюза на случай, если первые две защиты вдруг не сработают.

---

## 16. Trace-ID propagation (бонус)

Сквозной корреляционный трейсинг через все слои — observability-улучшение поверх `TASK.md` (не требуется заданием).
...
`trace_id` НЕ дублируется в `outbox_messages` — `OutboxPublisher` подтягивает его JOIN'ом из `notifications` при публикации.
...
CREATE TABLE notifications (
  ...
  recipient           VARCHAR(255) NOT NULL,                -- phone/email — он же идентификатор подписчика
  channel             VARCHAR(16)  NOT NULL,
  priority            VARCHAR(16)  NOT NULL,
  body                TEXT         NOT NULL,
  status              VARCHAR(16)  NOT NULL,
  status_history      JSONB        NOT NULL DEFAULT '[]',
  attempts            INTEGER      NOT NULL DEFAULT 0,      -- только НЕУДАЧНЫЕ попытки
  last_error          TEXT,
...
CREATE INDEX notifications_recipient_created_idx
  ON notifications (recipient, created_at DESC);              -- горячий путь read API
...
- Таблицу `subscribers` — идентификатор подписчика = его контакт; реестра подписчиков сервис не ведёт.
...
**Ответ 200:** см. описание формата ответа (массив `NotificationView` + `meta.next_cursor`).
...
`compose.yaml` (закрывает требование `TASK.md` про `docker compose up`):
...
3. `DispatchNotificationsTest` — идемпотентность входящих запросов.
4. `RetryTest` — transient retry + DLQ после исчерпания.
5. `Scenario8ReadApiTest` — read API.
6. `Scenario10DatabaseQueueRetryTest` — at-least-once для `SimulateDeliveryAckJob`.
7. `Scenario11TraceIdTest` — trace_id propagation.
8. `Scenario12OutboxRecoveryTest` — восстановление outbox после краша (центральный сценарий персистентности).
...
## 24. Привязка к `TASK.md` — что и каким приёмом закрывается

| Требование | Приём в коде |
|---|---|
| **Массовая рассылка уведомлений** | `POST /api/v1/notifications` → FormRequest → DTO → `DispatchNotificationsAction::handle()` в транзакции |
| **Приоритезация трафика** | Bulkhead через раздельные exchange-routing + раздельные пулы воркеров с разными CPU/mem |
| **Детализация статусов доставки** | `GET /api/v1/notifications?recipient=...`, индекс `(recipient, created_at DESC)`, cursor pagination |
| **Статусы: в очереди, отправлено, доставлено, отброшено** | State-машина в `Notification` (Queued → Sent → Delivered/Dropped), `status_history` JSONB |
| **Персистентность очереди (брокер + БД)** | Postgres durable + RabbitMQ durable + Transactional Outbox через `OutboxRepository` + `OutboxPublisher` с `FOR UPDATE SKIP LOCKED` |
| **Модель доставки: at-least-once** | Manual ack в `ConsumeNotificationJob` после `repo->save($n)`; database queue с `--tries=3` для ack-job |
| **Exactly-once на уровне бизнес-логики** | State check в `DeliverNotificationAction` + optimistic lock в `EloquentNotificationRepository::save()` + дедупликация шлюза через Redis Hash |
| **Retry-механизмы** | Per-message `expiration` + DLX-цикл `notifications.direct → retry-queue → notifications.direct`, max 5 попыток, backoff `1s,5s,25s,125s`, после исчерпания → `notifications.dlq` + `markAsDropped('max_retries_exceeded')` |
| **Дедубликация (Idempotency)** | `IdempotencyMiddleware` + `RedisIdempotencyStore`, ключ `idempotency:notifications.dispatch:{key}`, TTL 24ч |
| **Интеграционные тесты на основные сценарии** | `Scenario1E2ETest` + другие на реальном RabbitMQ/Postgres/Redis в том же `compose.yaml` |
| **Cloud Native и развертывание (Docker Compose)** | `compose.yaml` с healthchecks + one-shot `migrate` + `service_completed_successfully` зависимостями |
| **Публичный репозиторий** | Корень проекта |
| **Инструкция по запуску в README.md** | `README.md` |
| **Swagger (OpenAPI) / Описание API** | `src/resources/api-docs/openapi.yaml` + Swagger UI на `/api/docs` |
...
- Bearer / OAuth — auth не в `TASK.md`; сервис межсервисный во внутренней сети.
- Webhook от провайдера — `TASK.md` явно требует только заглушки.
- Kafka — RabbitMQ закрывает приоритезацию проще.
- Circuit Breaker / outbound rate limiter — не в `TASK.md`, future-proofing без обоснования.
- Event Sourcing — обычная state-машина проще.
- CQRS read-side projections — обычный read-репозиторий достаточен для эндпоинта истории.
...
2. **Прочитать архитектурное обоснование** в `DESCRIPTION.md`.
